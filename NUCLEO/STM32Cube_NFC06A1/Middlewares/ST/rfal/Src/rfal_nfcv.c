...
/*******************************************************************************/
// BEGIN rfalNfcvPollerTransceiveReq
ReturnCode rfalNfcvPollerTransceiveReq( uint8_t cmd,            // command code to be transmitted
                                        uint8_t flags,          // NFC type
                                        uint8_t param,          // optional parameter used for certain proprietary commands belonging to ST-Micro
                                        const uint8_t* uid,     // Unique Identifier of target NFC Device
                                        const uint8_t *data,    // data to be transmitted
                                        uint16_t dataLen,       // length of data to be transmitted
                                        uint8_t* rxBuf,         // buffer to hold reply after transmitting
                                        uint16_t rxBufLen,      // length of buffer to hold reply
                                        uint16_t *rcvLen        // placeholder for crc transmitted as part of reply
                                      )
{
    //constants
    ...
    rfalNfcvGenericReq req;         // a generic format for any type of NFC-V communications
    uint8_t            msgIt;       // a counter for traversing an array
    rfalBitRate        rxBR;        // receive bit rate
    bool               fastMode;    // fast mode flag

/**************************************** BEGIN FUNCTION ****************************************/
    // initialze msgIt to 0
    msgIt    = 0;

    // initialze fast mode flag to false
    fastMode = false;
    ...
    {
        // THEN return a parameter error code
        return ERR_PARAM;
    }
    // END IF
    
    /* IF the command is an ST's Fast command */
    ...
    {
        /* THEN Store current Rx bit rate and move to fast mode */
    ...
    /* Calculate Request Command */
    req.REQ_FLAG  = (uint8_t)(flags & (~((uint32_t)RFAL_NFCV_REQ_FLAG_ADDRESS)));
    req.CMD       = cmd;
    
    // IF the optional Proprietary Parameter is being used
    ...
    /* IF Request is to be sent in Addressed mode, not Select Mode ( Select Mode flag shall be set by user) */
    if( uid != NULL )
    {
        // THEN do some bitwise stuff with the Request Flag
        req.REQ_FLAG |= (uint8_t)RFAL_NFCV_REQ_FLAG_ADDRESS;

        // call ST's Memory Copy function to load the UID into the payload
        ST_MEMCPY( &req.payload.data[msgIt], uid, RFAL_NFCV_UID_LEN );

        //increment msgIt counter by the length of the UID so its pointing to the first index after the UID
        msgIt += RFAL_NFCV_UID_LEN;
    }
    // END IF
    
    // IF data length is greater than 0
    if( dataLen > 0U )
    {
        // THEN use ST's memory copy to add the data to the payload
        ST_MEMCPY( &req.payload.data[msgIt], data, dataLen);

        // increment msgIt counter by the length of the data so its pointing to the first index after the data
        msgIt += (uint8_t)dataLen;
    }
    // END IF
    
    /* Call Transceive Command to send the payload */
    ...
    // IF we had an arror at any point
    if( ret != ERR_NONE )
    {
    ...
    /* IF the response recieved is greater than the minimum response length */
    if( (*rcvLen) < (uint8_t)RFAL_NFCV_FLAG_LEN )
    {
        // Return the Error Code
        return ERR_PROTO;
    ...
    /* IF an error has been signalled in the Receive Buffer */
    if( (rxBuf[RFAL_NFCV_FLAG_POS] & (uint8_t)RFAL_NFCV_RES_FLAG_ERROR) != 0U )
    {
        // then parse the Error code and return it
        return rfalNfcvParseError( rxBuf[RFAL_NFCV_DATASTART_POS] );
    }
    
    // No errors detected so return No Error
    return ERR_NONE;
}
// END rfalNfcvPollerTransceiveReq
