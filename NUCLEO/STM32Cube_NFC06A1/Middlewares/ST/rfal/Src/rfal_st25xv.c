...
/*******************************************************************************/
// BEGIN rfalST25xVPollerWritePassword Function
ReturnCode rfalST25xVPollerWritePassword( uint8_t flags,        // Flags to be used
                                          const uint8_t* uid,   // UID of the target device
                                          uint8_t pwdNum,       // Password ID number
                                          const uint8_t *pwd,   // Password
                                          uint8_t pwdLen        // Password Length
                                        )
{
    // constants

    // variables
    uint8_t            data[RFAL_ST25xV_PWDNUM_LEN + RFAL_ST25xV_PWD_LEN];      // Array containing formatted password
    uint8_t            dataLen;                                                 // Length of data[]
    uint16_t           rcvLen;                                                  // holds CRC to validate successful transmission
    rfalNfcvGenericRes res;                                                     // structure representing a generic RF command


/********************************* BEGIN FUNCTION *********************************/

    // IF the password input parameters are invalid
    if( (pwdLen > RFAL_ST25xV_PWD_LEN) || (pwd == NULL) )
    {
        // THEN return a parameter error
        return ERR_PARAM;
    }
    // END IF
    ...
    // append password number to data array
    data[dataLen++] = pwdNum;

    // IF the length of the password is greater than 0
    if( pwdLen > 0U )
    {
        // THEN call ST's Memory Copy function to append the password into the data array
        ST_MEMCPY(&data[dataLen], pwd, pwdLen);
    }
    // END IF

    // increment data length by length of the password
    dataLen += pwdLen;

    // Return a call to rfalNfcvPollerTransceiveReq which will transmit the request
...
// END rfalST25xVPollerWritePassword Function
